...
public class WeekView extends View {
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
            ...
            switch (mCurrentFlingDirection) {
                ...
                case RIGHT:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) (velocityX * mXScrollingSpeed), 0, Integer.MIN_VALUE, Integer.MAX_VALUE,
                            (int) -(mHourHeight * 24 + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
                case VERTICAL:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, 0, (int) velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE,
                            (int) -(mHourHeight * 24 + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
            }
            ...
            if (mEventRects != null && mEventClickListener != null) {
                ...
                for (EventRect event : reversedEventRects) {
                    if (event.rectF != null && e.getX() > event.rectF.left && e.getX() < event.rectF.right && e.getY() > event.rectF.top && e.getY() < event.rectF.bottom) {
                        mEventClickListener.onEventClick((WeekViewEvent) event.originalEvent, event.rectF);
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (mEventLongPressListener != null && mEventRects != null) {
                ...
                for (EventRect event : reversedEventRects) {
                    if (event.rectF != null && e.getX() > event.rectF.left && e.getX() < event.rectF.right && e.getY() > event.rectF.top && e.getY() < event.rectF.bottom) {
                        mEventLongPressListener.onEventLongPress((WeekViewEvent) event.originalEvent, event.rectF);
    ...
    public WeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mEventTextSize = a.getDimensionPixelSize(R.styleable.WeekView_eventTextSize,
                    (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, mEventTextSize, context.getResources().getDisplayMetrics()));
            ...
            mLoadEventsInView = a.getBoolean(R.styleable.WeekView_loadEventsInView, mLoadEventsInView);
        } finally {
            a.recycle();
        }
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        canvas.clipRect(0, mHeaderTextHeight + mHeaderRowPadding * 2, mHeaderColumnWidth, getHeight(), Op.DIFFERENCE);
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        canvas.clipRect(mHeaderColumnWidth, mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2, getWidth(), getHeight(), Region.Op.DIFFERENCE);
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            if (mWidthPerDay + startPixel - start > 0) {
                if (mShowDistinctPastFutureColor) {
                    ...
                    canvas.drawRect(start, mHeaderTextHeight + mHeaderRowPadding * 2 + mTimeTextHeight / 2 + mHeaderMarginBottom, startPixel + mWidthPerDay, getHeight(),
                            sameDay ? mTodayBackgroundPaint : mDayBackgroundPaint);
                }
            }
        ...
        canvas.clipRect(mHeaderColumnWidth, 0, getWidth(), mHeaderTextHeight + mHeaderRowPadding * 2, Region.Op.DIFFERENCE);
     ...
     */
    protected void drawEvents(Calendar date, float startFromPixel, Canvas canvas) {
    ...
    protected void drawEventTitle(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        if (availableHeight >= lineHeight) {
            ...
            do {
                ...
                textLayout = new StaticLayout(TextUtils.ellipsize(bob, mEventTextPaint, availableLineCount * availableWidth, TextUtils.TruncateAt.END), mEventTextPaint,
                        (int) (rect.right - originalLeft - mEventPadding * 2), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
    ...
    /**
     * Gets more events of one/more month(s) if necessary.
     *
     * @param day The day where the user is currently is.
     */
    private void getMoreEvents(Calendar day) {
        getMoreEvents(day, mLoadEventsInView);
    }
     ...
     *
     * @param day        The day where the user is currently is.
     * @param loadInView loads the events from the results of the MonthChangeListener.
     */
    private void getMoreEvents(Calendar day, boolean loadInView) {
        if(loadInView) {
            ...
            if (mWeekViewLoader != null) {
                ...
                if (!isInEditMode() && (mFetchedPeriod < 0 || mFetchedPeriod != periodToFetch || mRefreshEvents)) {
                    ...
                    // Clear events.
                    mEventRects.clear();
                    //calculate event rects
                    EventListBuilder<Void,Void> builder = new EventListBuilder<>();
                    List<WeekViewEvent> events = new ArrayList<>();
                    events.addAll(previousPeriodEvents);
                    events.addAll(currentPeriodEvents);
                    events.addAll(nextPeriodEvents);
                    builder.addEvents(events);

                    setEvents(builder.getEventRects());
    ...
    public void setEvents(List<EventRect> events) {
        mEventRects = events;
        invalidate();
    }
